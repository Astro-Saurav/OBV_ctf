...
def extract_and_run(writeup_path, chal_dir, cat):
    ...
    for b_lang, b_code in blocks:
        # Ignore blocks that are just output or simple bash commands like "$ ./prog"
    ...
    # Replace <TARGET> for Web Exploitation
    if cat == "web_exploitation":
        port = chal_dir[-1] # challenge-0X-name -> port 800X
        port = f"800{port}" if port.isdigit() else "8001"
        script = script.replace("<TARGET>", f"localhost:{port}")
        script = script.replace("$TARGET", f"http://localhost:{port}")
...
for cat in categories:
    ...
    if cat == "web_exploitation":
        subprocess.run(["/tmp/pwntools_venv/bin/pip", "install", "aiohttp", "requests"], capture_output=True)
